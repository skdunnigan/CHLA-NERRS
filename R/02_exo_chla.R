...
chla_tbl <- chla_tbl %>%
  dplyr::select(datetime, date_sampled, chla_ugl, dil_fact_fluor)
...
data_dict <- data_dict %>%
  dplyr::mutate(date_sampled = as.Date(date_sampled, format = "%m/%d/%Y"))

# bind with chla_tbl
chla_tbl_long <- left_join(chla_tbl, data_dict, by = "date_sampled") %>%
